...
| `rate_unit_period` | Enum | Optional | The period of time that the `rate_unit` covers. It is "rolling" by default based on the current time from within the `rate_unit` (e.g. from the 8th of a "month" to the 8th of the next month, or from 17 minutes past the "hour" to 17 minutes past the next hour), or "calendar" for the start to the end of the current `rate_unit` (e.g.  from the 1st day of a month to the last day of the same month). One of "rolling", "calendar".  **Default** if not specified is "rolling". See [example](/curbs/examples.md#curb-policy-rate-units). | 
