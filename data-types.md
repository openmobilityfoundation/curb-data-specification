...
| ------ | ------ | ------------------- | ------------- |
| `data_source_url` | URL | Required | A web-accessible identifier for the source of the publicly or privately accessible data feed, document, website, or URL. This MUST be a full HTTPS URL pointing to a location which contains more information impacting or explaining the location, event, or policy. |
| `name` | String | Optional | Name of the data source for reference. E.g. "WZDx", "CWZ", "MDS", "GBFS", "GTFS", "CDS". |
| `public` | Boolean | Optional | Is this data source able to be viewed with out any sort of authentication? If `true`, the `data_feed_url` is public. If `false`, the `data_source_url` requires some sort of authentication, authorization, or API key to access. This is an informational field to set access expectations for the feed user, and does not provide any credentials directly unless explicitly contained in the `data_source_url` URL string. |
...
| `ids` | Array of Strings | Optional | An array of one or more **ids** from the data source that impacts the use of or relationship to part of CDS, e.g. a curb zone, curb space, curb area, curb event, etc. The **ids** and their details are be found in the referenced `data_source_url`. |
